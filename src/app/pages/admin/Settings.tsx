    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <header className="border-b border-[#dbe0ec] pb-7">
        ...
        <h1 className="font-['Source_Serif_4',serif] text-[40px] text-black tracking-[-1.2px]" style={{ lineHeight: 1.05 }}>
          Application<br />Settings
        </h1>
        <p className="font-['Source_Serif_4',serif] text-[#6c6c6c] text-lg tracking-[-0.3px] mt-2">
          Configure the application cycle, deadlines, and executive positions.
        </p>
      </header>
      ...
      <section>
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-['Radio_Canada_Big',sans-serif] font-medium text-black text-base">Cycle States</h2>
          <span className="font-['Geist_Mono',monospace] text-[11px] text-[#6c6c6c]">001</span>
        </div>

        <div className="border border-[#dbe0ec] space-y-0">
          {[
            { label: "Application Window", desc: "Allow students to submit new executive applications.", value: appWindowOpen, setter: setAppWindowOpen },
            { label: "Interview Scheduling", desc: "Enable applicants to book interview slots.", value: interviewsOpen, setter: setInterviewsOpen },
            { label: "Decisions Released", desc: "Make decision letters visible in applicant portal.", value: decisionsReleased, setter: setDecisionsReleased },
          ].map((item, i) => (
            <div key={item.label} className={`flex items-center justify-between px-6 py-5 ${i !== 0 ? "border-t border-[#dbe0ec]" : ""}`}>
              ...
              >
                <div className={`absolute top-1 w-4 h-4 bg-white transition-all ${item.value ? "left-7" : "left-1"}`} />
              </button>
            </div>
          ))}
        </div>
      ...
      <section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          <div>
            ...
            <input className={fieldCls} value={deadline} onChange={(e) => setDeadline(e.target.value)} placeholder="e.g. April 30, 2026" />
          </div>
          <div>
            <label className={labelCls}>Interview Window</label>
            <input className={fieldCls} value={interviewWindow} onChange={(e) => setInterviewWindow(e.target.value)} placeholder="e.g. May 5 – May 12, 2026" />
          </div>
          <div>
            <label className={labelCls}>Decisions Date</label>
            <input className={fieldCls} value={decisionsDate} onChange={(e) => setDecisionsDate(e.target.value)} placeholder="e.g. May 19, 2026" />
          </div>
        </div>
      </section>
        ...
        <button onClick={handleSave} disabled={saving} className="bg-black flex gap-[10px] items-center justify-center px-5 py-3.5 hover:bg-zinc-800 transition-colors disabled:opacity-50">
          {saving ? (
            ...
            <>
              ...
              <span className="font-['Geist_Mono',monospace] text-[13px] text-white whitespace-nowrap leading-none">Save Changes</span>
            </>
          )}
        </button>
      </div>
