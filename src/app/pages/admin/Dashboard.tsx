...
function PositionTags({ app, allPositions, onUpdate }: { app: any; allPositions: any[]; onUpdate: () => void }) {
  ...
  useEffect(() => {
    ...
    const handler = (e: MouseEvent) => {
      if (popRef.current && !popRef.current.contains(e.target as Node)) setShowAdd(false);
    };
    document.addEventListener("mousedown", handler);
    return () => document.removeEventListener("mousedown", handler);
  }, [showAdd]);

  const currentPositionIds = new Set(positions.map((ap: any) => ap.position_id));
  const removablePositions = allPositions.filter((p: any) => !currentPositionIds.has(p.id) && p.is_open);
  ...
  const handleAdd = async (positionId: string) => {
    setBusy(positionId);
    await supabase.from("application_positions").insert({ application_id: app.id, position_id: positionId });
    onUpdate();
    setBusy(null);
    setShowAdd(false);
  };
  ...
  return (
    <div className="flex flex-wrap items-center gap-1 relative">
      {positions.map((ap: any) => (
        <span
          ...
          className={cn(
            ...
            POS_STATUS_COLORS[ap.status] || POS_STATUS_COLORS.pending
          )}
        >
          {ap.positions?.title || "?"}
          {busy === ap.id ? (
      ...
      {positions.length === 0 && <span className="font-['Geist_Mono',monospace] text-[10px] text-[#6c6c6c]">&mdash;</span>}
          ...
          {showAdd && (
            <div className="absolute top-full left-0 mt-1 bg-white border border-[#dbe0ec] shadow-sm z-30 min-w-[160px]">
              {removablePositions.map((pos: any) => (
                ...
                >
                  {busy === pos.id ? <Loader2 className="w-3 h-3 animate-spin" /> : <Plus className="w-3 h-3 text-[#6c6c6c]" />}
                  {pos.title}
                </button>
              ))}
            </div>
...
export function AdminDashboard() {
  ...
  const handleQuickStatus = async (appId: string, newStatus: string) => {
    ...
    if (error) {
      ...
      if (app?.profiles?.email) {
        ...
        const positionNames = getPositionTitles(app) || "Executive Position";
        ...
        if (emailHtml) {
          supabase.functions.invoke("send-email", {
            body: { to: app.profiles.email, subject: emailSubject, html: emailHtml },
          }).catch(console.error);
        }
      }
    }
  ...
  const filteredApps = applications.filter((app: any) => {
    ...
    return name.includes(term) || email.includes(term) || positionText.includes(term);
  });

  // Group filtered apps by status
  const appsByStatus: Record<string, any[]> = {};
  for (const col of COLUMNS) {
    appsByStatus[col.key] = filteredApps.filter((app: any) => app.status === col.key);
  }

  // Drag handlers
  const onDragStart = (e: DragEvent<HTMLDivElement>, appId: string, currentStatus: string) => {
  ...
  const handleExportCSV = async () => {
    ...
    try {
      ...
      const userIds = filteredApps.map((a: any) => a.user_id).filter(Boolean);

      if (appIds.length === 0) return;

      // Fetch activities, responses with questions, and honors for all users
      const [activitiesRes, responsesRes, honorsRes, questionsRes] = await Promise.all([
        supabase.from("activities").select("*").in("user_id", userIds).order("sort_order"),
        supabase.from("responses").select("*, questions(prompt, position_id)").in("application_id", appIds),
        supabase.from("honors").select("*").in("user_id", userIds).order("sort_order"),
      ...
      (responsesRes.data || []).forEach((r: any) => {
        if (!responsesByApp[r.application_id]) responsesByApp[r.application_id] = [];
      ...
      const generalQuestions = allQuestions.filter((q: any) => !q.position_id);
      const positionQuestions = allQuestions.filter((q: any) => q.position_id);

      // Determine max counts for dynamic columns
      const maxActivities = Math.max(1, ...Object.values(activitiesByUser).map((a) => a.length));
      const maxHonors = Math.max(1, ...Object.values(honorsByUser).map((h) => h.length));

      // Build headers
      const headers: string[] = [
        "Name", "Email", "Grade", "Positions", "Status", "Submitted Date",
      ];

      // Activity columns
      for (let i = 1; i <= maxActivities; i++) {
        headers.push(`Activity ${i} Type`, `Activity ${i} Role`, `Activity ${i} Organization`, `Activity ${i} Description`, `Activity ${i} Hours/Week`, `Activity ${i} Weeks/Year`);
      }

      // Honor columns
      for (let i = 1; i <= maxHonors; i++) {
        headers.push(`Honor ${i} Title`, `Honor ${i} Grade Level`, `Honor ${i} Recognition Level`);
      }
      ...
      for (const app of filteredApps) {
        const name = `${app.profiles?.first_name || ""} ${app.profiles?.last_name || ""}`.trim() || "Unknown";
        ...
        const row: string[] = [name, email, grade, positions, status, submitted];

        // Activities
        const userActivities = activitiesByUser[app.user_id] || [];
        for (let i = 0; i < maxActivities; i++) {
          const a = userActivities[i];
          row.push(a?.type || "", a?.role || "", a?.organization || "", a?.description || "", String(a?.hours_per_week || ""), String(a?.weeks_per_year || ""));
        }
        ...
        for (let i = 0; i < maxHonors; i++) {
          ...
          row.push(h?.title || "", h?.grade_level || "", h?.recognition_level || "");
        }

        // Responses (match by question ID)
        const appResponses = responsesByApp[app.id] || [];
        const responseMap: Record<string, string> = {};
        appResponses.forEach((r: any) => { responseMap[r.question_id] = r.content || ""; });
        ...
        <div className="flex items-end justify-between">
          ...
          <div className="flex items-center gap-4 mb-1">
            <div className="flex border border-[#dbe0ec]">
              <button className={cn("px-3 py-1.5 font-['Geist_Mono',monospace] text-[11px]", view === "table" ? "bg-black text-white" : "text-[#6c6c6c] hover:text-black")} onClick={() => setView("table")}>Table</button>
              <button className={cn("px-3 py-1.5 font-['Geist_Mono',monospace] text-[11px]", view === "kanban" ? "bg-black text-white" : "text-[#6c6c6c] hover:text-black")} onClick={() => setView("kanban")}>Board</button>
            </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-[#dbe0ec]">
                <th className="px-4 py-3 text-left font-['Geist_Mono',monospace] text-[10px] text-[#6c6c6c] uppercase tracking-[0.06em] font-normal">Name</th>
                <th className="px-4 py-3 text-left font-['Geist_Mono',monospace] text-[10px] text-[#6c6c6c] uppercase tracking-[0.06em] font-normal">Email</th>
                <th className="px-4 py-3 text-left font-['Geist_Mono',monospace] text-[10px] text-[#6c6c6c] uppercase tracking-[0.06em] font-normal">Positions</th>
                <th className="px-4 py-3 text-left font-['Geist_Mono',monospace] text-[10px] text-[#6c6c6c] uppercase tracking-[0.06em] font-normal">Status</th>
                <th className="px-4 py-3 text-left font-['Geist_Mono',monospace] text-[10px] text-[#6c6c6c] uppercase tracking-[0.06em] font-normal">Submitted</th>
                <th className="px-4 py-3 text-left font-['Geist_Mono',monospace] text-[10px] text-[#6c6c6c] uppercase tracking-[0.06em] font-normal">Actions</th>
              </tr>
            </thead>
            <tbody>
              {filteredApps.length === 0 && (
                <tr>
                  <td colSpan={6} className="px-4 py-12 text-center font-['Geist_Mono',monospace] text-[11px] text-[#6c6c6c]">
              ...
              {filteredApps.map((app: any) => {
                ...
                return (
                  <tr key={app.id} className="border-b border-[#dbe0ec] hover:bg-[#f9f9f7]">
                    <td className="px-4 py-4">
                      <Link to={`/admin/applications/${app.id}`} className="font-['Radio_Canada_Big',sans-serif] font-medium text-sm text-black hover:underline">
                        {name}
                      </Link>
                    </td>
                    ...
                    <td className="px-4 py-4">
                      <PositionTags app={app} allPositions={allPositions} onUpdate={refetch} />
                    </td>
                    <td className="px-4 py-4">
                    ...
                    <td className="px-4 py-4">
                      {isUpdating ? (
                        ...
                        <select
                          value={app.status}
                          onChange={(e) => handleQuickStatus(app.id, e.target.value)}
                  ...
                  {columnApps.map((app: any) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className={cn(isUpdating && "invisible")}>
