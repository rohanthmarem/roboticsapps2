...
export function AdminQuestions() {
  ...
  useEffect(() => {
    supabase
      ...
      .order("sort_order")
      .then(({ data }) => {
        setQuestions((data || []).map((q: any) => ({ ...q, expanded: false })));
        setLoading(false);
      });
  }, []);
  ...
  const handleSave = async () => {
    ...
    if (errors.length > 0) {
      setError(errors.join(" | ") + " — Have you run supabase-rls-fix.sql in Supabase SQL Editor?");
    }
    setSaving(false);
  };

  // Count questions per tab
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-wrap gap-2">
        <button
          ...
          className={cn(
            "px-4 py-2 font-['Geist_Mono',monospace] text-[12px] border transition-colors",
            activeTab === "all" ? "bg-black text-white border-black" : "bg-white text-black border-[#dbe0ec] hover:border-black"
          )}
        >
        ...
        <button
          ...
          className={cn(
            "px-4 py-2 font-['Geist_Mono',monospace] text-[12px] border transition-colors",
            activeTab === "general" ? "bg-black text-white border-black" : "bg-white text-black border-[#dbe0ec] hover:border-black"
          )}
        >
        ...
        {positions.map((p: any) => (
          <button
            ...
            className={cn(
              "px-4 py-2 font-['Geist_Mono',monospace] text-[12px] border transition-colors",
              activeTab === p.id ? "bg-black text-white border-black" : "bg-white text-black border-[#dbe0ec] hover:border-black"
            )}
          >
      ...
      {filteredQuestions.length === 0 ? (
        ...
        <div className="space-y-3">
          <Reorder.Group axis="y" values={filteredQuestions} onReorder={(newOrder) => {
            // Reorder only within filtered view, preserve other questions
                      ...
                      <div className="flex items-center gap-4 flex-1 min-w-0 pr-4">
                        ...
                        <div className="min-w-0">
                          <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                            ...
                            <span className={cn(
                              "font-['Geist_Mono',monospace] text-[10px] border px-1.5 py-0.5",
                              q.position_id ? "text-black border-black" : "text-[#6c6c6c] border-[#dbe0ec]"
                            )}>
                              {q.position_id ? positions.find((p: any) => p.id === q.position_id)?.title || "Position" : "General"}
                            </span>
                            {!q.is_active && (
                              <span className="font-['Geist_Mono',monospace] text-[10px] text-[#6c6c6c] border border-[#dbe0ec] px-1.5 py-0.5">Inactive</span>
                            )}
                            {q.is_required && (
                              <span className="font-['Geist_Mono',monospace] text-[10px] text-black">Required</span>
                            )}
                          </div>
                      ...
                      <div className="px-5 pb-5 pt-4 border-t border-[#dbe0ec] bg-[#f9f9f7] space-y-5">
                        <div>
                          ...
                          <textarea
                            ...
                            placeholder="e.g. Why are you interested in this executive position?"
                          />
                        </div>
                        ...
                        <div>
                          ...
                          <input
                            ...
                            placeholder="e.g. Tell us what motivates you..."
                          />
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                          ...
                          <div>
                            <label className={labelCls}>Character Limit</label>
                            <input
                              type="number"
                              className={fieldCls}
                              value={q.char_limit}
                              onChange={(e) => updateField(q.id, "char_limit", parseInt(e.target.value) || 0)}
                              min={0}
                            />
                          </div>
                          ...
                          <div>
                            ...
                            <select className={selectCls} value={q.position_id || ""} onChange={(e) => updateField(q.id, "position_id", e.target.value || null)}>
                              ...
                              {positions.map((p: any) => (
                                <option key={p.id} value={p.id}>{p.title} — Specific</option>
                              ))}
                            </select>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-6 pt-2 border-t border-[#dbe0ec]">
                          <label className="flex items-center gap-2 cursor-pointer">
                            <input
                              ...
                              onChange={(e) => updateField(q.id, "is_required", e.target.checked)}
                            />
                          ...
                          <label className="flex items-center gap-2 cursor-pointer">
                            <input
                              ...
                              className="w-4 h-4"
                              checked={q.is_active}
                              onChange={(e) => updateField(q.id, "is_active", e.target.checked)}
                            />
